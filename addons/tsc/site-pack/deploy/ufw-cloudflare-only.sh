#!/usr/bin/env bash
# Allow HTTP/HTTPS ONLY from Cloudflare edges, so the origin cannot be reached around the proxy.
# SSH is untouched. Revert: sudo ufw allow 80/tcp && sudo ufw allow 443/tcp
set -e
